// Směnářek 1.8.1 – KOMPLETNÍ PŮVODNÍ KÓD + MADETA ÚPRAVY
...
function bindInputsOnce(){
  ...
  ['rate_base','rate_odpo','rate_noc','rate_vikend','rate_nepretrzity'].forEach(id=>{
    ...
    // Nastavení výchozích Madeta hodnot, pokud jsou prázdné
...
function updateStats(){
  ...
  $('stats').innerHTML = `Ranní: <b>${rDays}</b> • Denní: <b>${dDay}</b> • Noční: <b>${nDay}</b> • Dovolené: <b>${vac}</b><br>Hodiny: <b>${r2(hours)}</b><br>Svátek odpracovaný: <b>${r2(holWorkedH)} h</b>`;
...
function computeDailyBreakdown(){
  ...
  for(let i=1;i<=last;i++){
    ...
    if(t==='R'){
        ...
        worked=11.25; afterH+=3.75;
        if(isW(dt)) weekendH+=11.25;
        if(isHoliday(dt)) holH+=11.25;
    } else if(t==='N'){
        worked=11.25; afterH+=4.00; nightH+=7.25;
        const wd=dt.getDay();
        if(wd===5) weekendH+=6;
