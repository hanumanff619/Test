...
// ✅ 8h: R má 2 varianty (Po–Pá / So–Ne), popisek obecný (konkrétní čas v "Dnes")
...
const isW=d=>[0,6].includes(d.getDay());        // So/Ne
...
g=Math.floor((b-f+1)/3), h=(19a+b-d-g+15)%30, i=Math.floor(c/4), k=c%4,
l=(32+2e+2i-h-k)%7, m=Math.floor((a+11h+22l)/451),
month=Math.floor((h+l-7m+114)/31), day=1+((h+l-7*m+114)%31);
...
// přímé prémie a roční motivační
...
// po přepnutí měsíce doplnit hodnoty měsíčních polí
...
// spočítat statistiky měsíce (akumulační)
...
let roCount=0; // ✅ počet R+O směn (jen pro statistiku)
...
if(state.mode==='8'){  
  ...
  if(t==='O'){  
    hours += h;  
    roCount++;  
    afterH += h;               // ✅ odpolední příplatek jen za O  
    if(isW(dt)) weekendH += h;  
  }  
  if(t==='N'){  
    hours += h;  
...
? Ranní+Odpolední: <b>${roCount}</b> • Noční: <b>${r2(nightH/7.25)}</b> • Dovolené: <b>${vac}</b>
...
$('substats').innerHTML = [
<div class="payline"><span>Odpolední hodiny (D: 3.75, N: 4.00)</span><span><b>${r2(afterH)}</b> h</span></div>,
<div class="payline"><span>Noční hodiny (22–6) — 7.25/h</span><span><b>${r2(nightH)}</b> h</span></div>,
...
// afterH = odpolední hodiny (pro příplatek) => jen O  
    if(t==='R'){  
      nightH += rShiftNightH(dt);  
    }  
    if(t==='O'){  
...
// základ z měsíční mapy (pokud existuje), jinak globální
...
if(state.mode==='12'){  
    ...
    // ✅ 8h: 1 stravenka pouze za sobotu, jinak 0  
    if(isSat(dt) && (t==='R' || t==='O' || t==='N')) count += 1;  
    // lunches zůstává 0  
  }  
}  
...
const mc=mealsCalc();
const mealDeduct = mc.countMEAL_DEDUCT, lunchDeduct=mc.lunchesLUNCH_DEDUCT, mealValue=mc.count*MEAL_INFO_VALUE;

const gross = basePay+odpoPay+nightPay+wkPay+holPay+nepret+prime+vacPay + annualBonus + fund + satBonus;
const social=gross0.065, health=gross0.045;
...
// roční přehled – ulož i stravenky pro součty
...
// pokud je audit otevřený, překreslit
...
box.innerHTML =   <hr>   <div class="payline"><span>Součet hrubé (rok ${y})</span><span><b>${money(sumGross)}</b></span></div>   <div class="payline"><span>Součet čisté (rok ${y})</span><span><b>${money(sumNet)}</b></span></div>   <div class="payline"><span>Součet stravenek (rok ${y})</span><span><b>${sumMealCount} ks — ${money(sumMealVal)}</b></span></div>   <div class="subtle">Započítáno měsíců: ${months.length}</div>  ;
}
